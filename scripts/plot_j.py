...
def get_trajectory_2d(odoms, stamps_, stack_size):
...
model_file_yaw = '/Users/alexander/Development/kitti_vo/models/james/model_odom.0081-0.005593-0.000726.h5'
...
seq_name = '05' if len(sys.argv) == 1 else sys.argv[1]

scalers_fname = os.path.join(kitti_dir, 'scripts', 'scalers.pickle')

scalers = pickle.load(open(scalers_fname, 'rb'))

image_paths, stamps, odoms = filename_loaders.load_filenames_odom(dataset_dir, stack_size, [seq_name])

image_paths, stamps_stacks, odom_stacks = train.stack_data(image_paths, stamps, odoms, stack_size)

image_stacks, _ = train.load_image_stacks(image_paths)

odom_gt = np.array([filename_loaders.poses_to_offsets(p, ['y', 'yaw']) for p in odom_stacks])
...
model_yaw = load_model(model_file_yaw, custom_objects={'weighted_mse': train.weighted_mse})
