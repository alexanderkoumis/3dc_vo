...
void saveErrorPlots(vector<errors> &seq_err,string plot_error_dir,char* prefix) {
  ...
  std::vector<float> t_errs;
  std::vector<float> r_errs;
  ...
  for (int32_t i=0; i<num_lengths; i++) {
    ...
    if (num>2.5) {
      fprintf(fp_tl,"%f %f\n",lengths[i],t_err/num);
      fprintf(fp_rl,"%f %f\n",lengths[i],r_err/num);
      t_errs.push_back(t_err/num);
      r_errs.push_back(r_err/num);
    }
  }

  float t_err = std::accumulate(std::begin(t_errs), std::end(t_errs), 0.0) / t_errs.size();
  float r_err = std::accumulate(std::begin(r_errs), std::end(r_errs), 0.0) / r_errs.size();
  fprintf(fp_tl, "Mean: %f\n", t_err);
  fprintf(fp_rl, "Mean: %f\n", r_err);
...
bool eval (string prefix, string result_sha, Mail* mail, string gt_dir, string result_dir) {
  ...
  int32_t sequence_nums[] = {3, 4, 5, 6, 7, 10};
  for (int32_t i : sequence_nums) {
    ...
    if (!loadPoses(gt_dir + "/" + file_name, poses_gt) || !loadPoses(result_dir + "/data/" + file_name, poses_result))
    {
      continue;
    }
    // plot status
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  string prefix = "";
  string gt_dir_ = argv[2];
  string results_dir_ = argv[3];
  ...
  // run evaluation
  bool success = eval(prefix, result_sha,mail, gt_dir_, results_dir_);
